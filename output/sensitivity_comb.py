...
def get_param_name(dir_path):
    """
    Infers parameter name from directory.
    Ex: 'sensitivity_budget' -> 'budget'
    """
...
def parse_filename(filename, param_name):
    """
    Extracts model ID and param value.
    Ex: 'mod1_budget100_6.csv' -> Model: 1, Budget: 100
    """
        ...
        if mod is not None and val is not None:
            ...
            try:
                ...
                if 'step' in df.columns and 'l2' in df.columns:
                    # STRICTLY KEEP ONLY step and l2
                    df = df[['step', 'l2']].copy()
